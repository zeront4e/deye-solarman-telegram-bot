...
public class WattMonthStatisticsBotCommand implements BotCommand {
    private final DbDailyMeasuringPointsManager dbDailyMeasuringPointsManager;

    public WattMonthStatisticsBotCommand(DbDailyMeasuringPointsManager dbDailyMeasuringPointsManager) {
    ...
    public void onCommandRead(List<String> commandArguments, Update update, TelegramBot telegramBot) throws Exception {
        ...
        if(imageBytes == null) {
            SendMessage sendMessage = new SendMessage(update.message().chat().id(), "I'm sorry, but I don't " +
                    "have any data for this month. " + Constants.Emojis.SAD + " Please try again later.");
        ...
        sendPhoto.caption("Here is the kWh consumption data from this month. " + Constants.Emojis.STATISTICS);
