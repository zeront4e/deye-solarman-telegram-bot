...
public class WattYesterdayStatisticsBotCommand implements BotCommand {
    private final DbBasicMeasuringPointsManager dbBasicMeasuringPointsManager;

    private final int maxMeasuringPointsPerHour;

    public WattYesterdayStatisticsBotCommand(DbBasicMeasuringPointsManager dbBasicMeasuringPointsManager,
    ...
    public void onCommandRead(List<String> commandArguments, Update update, TelegramBot telegramBot) throws Exception {
        ...
        byte[] imageBytes = BasicMeasurementDataChartsUtil.generateDayHoursWattChartOrNull(
                dbBasicMeasuringPointsManager, yesterdayLocalDateTime, maxMeasuringPointsPerHour);
        ...
        if(imageBytes == null) {
            SendMessage sendMessage = new SendMessage(update.message().chat().id(), "I'm sorry, but I don't " +
                    "have any data from yesterday. " + Constants.Emojis.SAD + " Please try again later.");
        ...
        sendPhoto.caption("Here is the Watt data from yesterday. " + Constants.Emojis.STATISTICS);
